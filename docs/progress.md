...
**Status:** Phase 3 done. Phase 4 **unit A (settings)** + **unit B (recycled list holders)** done, committed & pushed. **Next = Phase 4 unit C (non-recycled title sites).** Read `docs/dearrow-design.md` §3.4–§3.5 (recycled-async pattern) and §4 (hook sites) before coding.
...
- [ ] Baseline build: confirm upstream code builds unmodified under Nix (`nix run .#build`)
...
- [ ] Wire the non-recycled title sites (detail/player/dialog/queue) — unit C
...
|-------|--------|----------|
| Phase 1: Foundation — Nix toolchain, branching & submodule wiring | In Progress (build deferred) | 6/7 |
| Phase 2: Map SponsorBlock & design DeArrow | Done | 4/4 |
| Phase 3: DeArrow service in the Client | Done (gradle test run pending) | 5/5 |
| Phase 4: DeArrow titles in Client | In Progress | 2/6 |
...
## Deferred / To verify
- **Baseline build** (`nix run .#build`) — flake is written and `nix eval`-validated, but the first real build (~10–40 min: downloads SDK + JDK 11 + Gradle + deps) has NOT been run. Deferred by user.
