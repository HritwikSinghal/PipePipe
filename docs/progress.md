...
**Status:** **Phase 1 build WORKS** (`nix run .#build` → 5 debug APKs; Session 6 JDK swap). **Phase 4 is code-complete (5/6)** — units A/B/C + the new "Mark replaced titles" indicator are all committed & pushed (client `patch` HEAD `d6e6bcaa8`; meta gitlink bumped). The only open Phase-4 item is **manual on-device verification, which the user will do via the GitHub release pipeline + Obtainium** — so it's gated on **Phase 6 (signing + distinct applicationId) and Phase 7 (release workflow)**. **Next = either Phase 5 (DeArrow thumbnails) or jump to Phase 6/7 to get an installable signed APK for verification.** Decide with the user. (Build verified each step: assembleDebug + dearrow unit tests 23/23. `runCheckstyle` is NOT a gate — upstream fails it with 2192 errors and CI doesn't run it; our dearrow files add 0 errors / 1 unavoidable override warning.)
