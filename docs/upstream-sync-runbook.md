...
All DeArrow unit tests must pass (63 as of v5.2.5). Checkstyle is **not** in
`assembleDebug` and **not** a CI gate, and upstream itself fails it -- hand-check
any new file for the 100-column limit, `final` params, unused imports, and a
trailing newline.
...
```sh
git diff main patch -- $(git ls-tree main --name-only | grep -vE 'README.md|PipePipeClient|PipePipeExtractor')
git diff <old-meta-tip> patch -- flake.nix flake.lock .github docs CLAUDE.md .gitmodules
```

The first proves we dropped nothing of upstream's; the second proves our own
content survived. `README.md` is expected to differ on both sides (we rewrote it,
and upstream edits its own sections) -- diff it by hand.
